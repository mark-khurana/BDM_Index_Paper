...
#Loading empirical tree index values for analysis -----------------------------------------------------------------------------------------------------
load(file="finaltreeparams.RData")

#Wilcoxon Rank Sum Test, comparing empirical with simulated trees ------------------------------------------------------------------------------------------------------------------
library('dplyr')
empirical <- filter(finaltreeparams, EmpOrSim == "emp")
simulated <- filter(finaltreeparams, EmpOrSim == "sim")
...
wil_all <- list(wil_TotalI, wil_MeanI, wil_MedianI, wil_NormColless, wil_CollessLikeExpMDM, wil_I2, wil_NormSackin, 
...
save(wil_all, file="wilcoxon_values.RData")
load(file="wilcoxon_values.RData")

#combining all wilcoxon information into a dataframe
wilcoxon_distance_values <- data.frame(
...
save(wilcoxon_distance_values, file="wilcoxon_distance_values.RData")
load(file="wilcoxon_distance_values.RData")

#Z score comparisons-----------------------------------------------------------------------------------------------------------
zscores <- list("treenumber", "gamma", "TotalI","MeanI", "MedianI", "NormColless", "CollessLikeExpMDM", 
...
zscoresdf <- data.frame(zscores$treenumber, zscores$gamma, zscores$TotalI, zscores$MeanI, zscores$MedianI, zscores$NormColless, 
...
colnames(zscoresdf) <- c("treenumber", "gamma", "TotalI","MeanI", "MedianI", "NormColless", "CollessLikeExpMDM", 
                        "I2", "Sackin", "NormSackin", "TotalCophenetic", "B1", "B2", "AvgLeafDepth",
                        "leafdepthvariance", "CPrank", "normcherry", "Jindex", "SNI", "Sshape",
                        "APP", "normILnumber", "normaverageladder", "maxdepth", "maxwidth", 
                        "maxdiffwidth", "meandepth", "maxheight",  
                        ...
                        "stairs", "stairs2", "longestpendant", "shortestpendant", "stemminess")
save(zscoresdf, file="zscoresdf.RData")
